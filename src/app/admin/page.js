...
const [aiLoading, setAiLoading] = useState(false);
  ...
  const downloadCSV = () => {
    ...
    link.setAttribute("download", `Analytics_Report_${filters.range}_${new Date().toLocaleDateString()}.csv`);
      ...
      <header className="sticky rounded-2xl top-0 z-50 px-4 md:px-8 pt-8 md:pt-10 pb-6 bg-white/60 backdrop-blur-2xl border-b border-white/20">
        <div className="max-w-7xl mx-auto flex flex-col gap-6">
          <div className="flex justify-between items-center">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-600 mb-1 leading-none">Intelligence</p>
    ...
    {aiLoading ? "Analyzing…" : "AI Summary"}
              ...
              {["TODAY", "MONTH", "YEAR"].map((r) => (
                <button
                  ...
                  className={`flex-1 md:flex-none px-6 py-2 rounded-xl text-[11px] font-bold transition-all duration-300 ${
                    filters.range === r ? "bg-black text-white shadow-lg" : "text-slate-500 hover:text-slate-700"
                  }`}
                >
                  {r}
                </button>
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 pt-8 space-y-6 md:space-y-8 relative z-10">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          <KPI label="Distributors" value={kpis.distributors} icon={<Users size={18}/>} color="bg-blue-500" />
          <KPI label="Active Today" value={kpis.activeToday} icon={<Zap size={18}/>} color="bg-emerald-500" />
          <KPI label="Meetings" value={kpis.meetings} icon={<TrendingUp size={18}/>} color="bg-violet-500" />
          <KPI label="Samples" value={kpis.samples} icon={<Package size={18}/>} color="bg-amber-500" />
      ...
      {parseAISummary(aiSummary).map((sec, idx) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <ChartCard title="Product-wise Sales">
            <BarChartBox data={productSales} barKey="quantity" color="#10b981" />
          </ChartCard>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <ChartCard title="Meeting → Sale Conversion">
            <BarChartBox data={data.conversionFunnel || []} barKey="value" color="#3b82f6" />
          </ChartCard>
          <ChartCard title="Sample → Sale Conversion">
            <BarChartBox data={data.conversionFunnel || []} barKey="value" color="#f59e0b" />
          </ChartCard>
        </div>
          ...
          <div className="h-[400px] w-full mt-6">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={distributorPerformance} barGap={8}>
                ...
                <Bar dataKey="meetings" fill="#3b82f6" radius={[4, 4, 0, 0]} barSize={20} />
                <Bar dataKey="sales" fill="#10b981" radius={[4, 4, 0, 0]} barSize={20} />
                <Bar dataKey="distance" fill="#94a3b8" radius={[4, 4, 0, 0]} barSize={20} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </ChartCard>
    ...
    <div className="min-h-screen bg-[#F2F2F7] p-8 space-y-8 animate-pulse">
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="h-32 bg-white/60 rounded-[2.5rem] border border-white" />
          ))}
        </div>
...
function KPI({ label, value, icon, color }) {
  return (
    <div className="group bg-white/40 backdrop-blur-3xl border border-white/60 p-6 rounded-[2.5rem] shadow-[0_8px_32px_rgba(0,0,0,0.03)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.06)] transition-all duration-500">
      <div className={`${color} w-11 h-11 rounded-2xl flex items-center justify-center text-white mb-5 shadow-lg shadow-inherit/20`}>
        {icon}
      </div>
...
function ChartCard({ title, children }) {
  return (
    <div className="bg-white/40 backdrop-blur-3xl border border-white/60 p-8 rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.04)]">
      <h2 className="text-xl font-black tracking-tight text-slate-900 mb-8">{title}</h2>
      {children}
    </div>
  );
}
    ...
    <div className="h-[250px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data}>
          ...
          <Bar dataKey={barKey} fill={color} radius={[8, 8, 0, 0]} barSize={32} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-slate-900/95 backdrop-blur-md text-white p-4 rounded-2xl shadow-2xl border border-white/10 ring-1 ring-white/20">
        <p className="text-[9px] font-black uppercase tracking-widest text-white/40 mb-1">{payload[0].name || payload[0].payload.name}</p>
